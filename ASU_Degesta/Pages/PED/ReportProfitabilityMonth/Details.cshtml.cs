...
namespace ASU_Degesta.Pages.PED.ReportProfitabilityMonth
{
    public class DetailsModel : PageModel
    {
        ...
        public IList<Models.PED.ReportProfitabilityMonth> ReportProfitabilityMonth { get; set; } = default!;
        public List<TypesOfProducts> TypesOfProductsList;
        public List<Units> UnitsList;
        public List<ReportProfitabilityMonth_id> info { get; set; } = default!;
        ...
        public async Task<IActionResult> OnGetAsync(string? id)
        {
            ...
            info = await _context.ReportProfitabilityMonth_id.Where(x => x.doc_id == id).ToListAsync();
            if (info.Count == 0)
            {
                return NotFound();
            }

            ReportProfitabilityMonth =
                await _context.ReportProfitabilityMonth.Where(x => String.Equals(x.doc_id, id)).OrderBy(x=>x.types_of_products_id).ToListAsync();
