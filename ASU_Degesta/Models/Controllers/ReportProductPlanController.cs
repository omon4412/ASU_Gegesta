...
[ApiController]
[Route("api/GetReportProductPlanDocx")]
public class ReportProductPlanController : Controller
{
    public ActionResult OnGet([FromBody] ReportProductPlanData data)
    {
                   ...
                   DocumentFormat.OpenXml.WordprocessingDocumentType.Document, true))
        {
            ...
            foreach (var item in datas)
            {
                data_table.Add(new List<string>()
                {
                    ...
                    item.produced.ToString(), data.UnitsList.Where(x=>x.Units_ID == item.units_id).FirstOrDefault().Name
                });
            }
    ...
    public class ReportProductPlanData
    {
        public List<ReportProductPlan> Reports { get; set; }
        public ReportProductPlan_id Report_ID { get; set; }
