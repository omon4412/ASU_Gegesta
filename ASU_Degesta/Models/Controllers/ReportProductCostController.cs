...
[ApiController]
[Route("api/GetReportProductCostDocx")]
public class ReportProductCostController : Controller
{
    public ActionResult OnGet([FromBody] ReportProductCostData data)
    {
                   ...
                   DocumentFormat.OpenXml.WordprocessingDocumentType.Document, true))
        {
            ...
            {
                "Наименование", "Себестоимость", "Единицы измерения"
            });
            foreach (var item in datas)
            {
                data_table.Add(new List<string>()
                {
                    ...
                    item.cost_price.ToString(), data.UnitsList.Where(x=>x.Units_ID == item.units_id).FirstOrDefault().Name
                });
            }
    ...
    public class ReportProductCostData
    {
        public List<ReportProductCost> Reports { get; set; }
        public ReportProductCost_id Report_ID { get; set; }
